...
## NECESSARY IMPORT STATEMENTS DO NOT REMOVE ##
# Streamlit is  an open-source Python framework 
# that allows you to turn data scripts into interactive, shareable web applications
import streamlit as st


# Data sources from PNWU
...
# Page configuration (set the title and PNWU logo)
st.set_page_config(
...
# Additional page styling/visual requirements
st.markdown("""
<style>
...
# Create the individual tabs for the homepage and bot, and also footer
with st.sidebar:
...
# Create dataframes for books, satisfaction survey, and circulation materials
...
# Display individual numeric metrics for each of the listed categories
...
# To be used in a future installment, rough layout used from stackoverflow

"""
    with t2:
        st.subheader("Collection Value")
        st.caption("Physical book circulation from ___, AY21-22 through AY24-25.")
...
# Also to be added in a future installment
    with t3:
...
# From satisfaction surveys beforehand
    with t4:
...
# Direct quotes usef from surveys
    with t5:
...
# Insight Bot Page
    ...
    st.caption(
        "Ask about Book a Librarian appointments or the student satisfaction "
        "surveys. Answers are computed directly from the dashboard data — the "
    ...
    if "bot_messages" not in st.session_state:
        st.session_state.bot_messages = [
            {"role": "assistant",
             "content": "Hi! Ask me about library appointments or satisfaction surveys."}
        ]
